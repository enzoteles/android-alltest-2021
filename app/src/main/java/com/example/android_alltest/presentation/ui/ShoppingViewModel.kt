package com.example.android_alltest.presentation.ui
...
import com.example.android_alltest.data.util.Event
    ...
    fun insertShoppingItem(name: String, amountString: String, priceString: String) {
        ...
        if(name.length > Utils.MAX_NAME_LENGTH) {
            _insertShoppingItemStatus.postValue(Event(Resource.error("The name of the item" +
                    "must not exceed ${Utils.MAX_NAME_LENGTH} characters", null)))
            return
        }
        if(priceString.length > Utils.MAX_PRICE_LENGTH) {
            _insertShoppingItemStatus.postValue(Event(Resource.error("The price of the item" +
                    "must not exceed ${Utils.MAX_PRICE_LENGTH} characters", null)))
            return
        }
        val amount = try {
            amountString.toInt()
